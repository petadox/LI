...
writeClauses:-
	onePlacePerRect,			% Fiquem un sol cop cada rect
	initFillsVar,				% Omplim la variable fills
	%noOverlapRects,			% No overlapping
	noOverlapRects2,			% No overlapping (menys clausules)
    true.
...
onePlacePerRect:-
    ...
    findall(starts-B-X-Y,(insideTable(X,Y), Aux1 is X+W-1, Aux2 is Y+H-1, xCoord(Aux1), yCoord(Aux2)),Lits),
    exactly(1,Lits),
    fail.
onePlacePerRect.
...
initFillsVar:-
	...
	width(B,W), height(B,H),
    insideTable(X,Y),
    Aux1 is X+W-1, Aux2 is Y+H-1,
    xCoord(Aux1), yCoord(Aux2),
    between(X,Aux1,X2), between(Y,Aux2,Y2),
    writeClause( [\+starts-B-X-Y, fills-B-X2-Y2] ), fail.
initFillsVar.

noOverlapRects:-
	rect(B1), rect(B2), B1 \= B2,
    insideTable(X,Y),
    writeClause( [\+fills-B1-X-Y, \+fills-B2-X-Y] ), fail.
noOverlapRects.
...
displaySol(M):- nl, height(H), yCoord(Z), nl, Y is H-Z+1, xCoord(X), member(fills-B-X-Y, M), write(B), espais(B), fail.
displaySol(_):- nl.
