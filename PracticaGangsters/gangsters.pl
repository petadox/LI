...
%   2. work-G-H means: "gangster  G works on hour H"

writeClauses(K):-
    initClauseGeneration,
    eachHourEachGangsterAtMostOneTask,
    noDifferentsTasksConsecutiveHours,
    eachHourEachTaskExactlyKGangsters,
    initWorkVar,
    atMostKHoursWorkingEachGangster(K),
    true,
    !.
...
atMostKHoursWorkingEachGangster2(K):-
	hour(H),
	Hk is H+K,
	Hk =< 72,
	gangster(G),
	findall(work-G-H2, between(H,Hk,H2), Lits), negateAll(Lits,NLits), writeClause(NLits),
	fail.
atMostKHoursWorkingEachGangster2(_).

atMostKHoursWorkingEachGangster(K):-
	available(G,H1),
	Aux is H1+K,
	hour(Aux),
	findall(work-G-H, (between(H1,Aux,H),available(G,H)), Lits),
	atMost(K,Lits),
	fail.
atMostKHoursWorkingEachGangster(_).

maxConsecutiveHours2(M,K) :- 
	between(0,71,N), 
	K is 72-N,
    gangster(G),
    findall(H, member(work-G-H,M), L),
    sort(L, Lsorted),
    append(_,L1,Lsorted), 
    append(S,_,L1),
    length(S,K),
...
main:-
    ...
    tell(clauses), writeClauses(20), told,     % generate the (numeric) SAT clauses and call the solver
