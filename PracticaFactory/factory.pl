...
%   2. used-T-R-H means: "task T uses resource R at hour H"		(MANDAHUEVOS)

writeClauses(Time):- 
    initClauseGeneration,
    eachTaskStartsOnce(Time),
    eachTaskHourValid(Time),
    %noMoreResources(Time),
    true,!.
        
%%%%%%%%%%%%%%%%%%%%% Auxiliary predicates for displaying the solutions and for counting the hours used:

slot(S,Time):- between(1,Time,S).

eachTaskStartsOnce(Time):-
	task(T),
	duration(T,D),
	Aux is Time-D,
	findall(start-T-H,slot(H,Aux),Lits),
    exactly(1,Lits),
    fail.
...
noMoreResources(Time):-
	slot(H,Time),
	resourceUnits(R,U),
	findall(used-T-R-H, task(T), Lits),
	atMost(U,Lits),
	fail.
noMoreResources(_).
...
canviVars(Time):-
	...
	slot(H,Time),
	usesResource(T,R),
	writeClause([\+start-T-H, used-T-R-H]), 
	fail. 
canviVars(_).	
